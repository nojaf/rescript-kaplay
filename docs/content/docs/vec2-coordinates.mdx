...
<Aside type="tip">
  Always use the Context API to create vectors. Never construct Vec2 records
  manually!
</Aside>
...
<Aside type="caution">
  You cannot mix coordinate systems in operations. Adding `Vec2.Local.t` to
  `Vec2.World.t` will not compile.
</Aside>

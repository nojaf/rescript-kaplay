...
<Aside type="caution">
**Creating Unit modules is uncommon**: Most of the time, you'll use existing `Unit` modules from the library or create custom combined modules for your domain. Creating a `Unit` module for a single component (like `Attack.Unit`) is typically only done when that component is central to your game's architecture and needs to be queried generically across many different object types.
</Aside>

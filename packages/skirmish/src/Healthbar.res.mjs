...
function make(pokemon) {
  let healthbar = GameContext$Skirmish.k.add([
    {
      healthPercentage: Pkmn$Skirmish.getHealthPercentage(pokemon),
      name: Stdlib_Option.getOr(MetaData$Skirmish.names.get(pokemon.pokemonId), "???"),
      level: pokemon.level,
      team: Pkmn$Skirmish.getTeam(pokemon),
      pokemon: pokemon
    },
    {
      id: "healthbar",
    ...
    Pkmn$Skirmish.getTeam(pokemon) === false ? GameContext$Skirmish.k.pos(10, 10) : GameContext$Skirmish.k.pos(0, GameContext$Skirmish.k.height() - playerHeight)
  ]);
  pokemon.onHurt(_deltaHP => {
    let newHealthPercent = Pkmn$Skirmish.getHealthPercentage(pokemon);
